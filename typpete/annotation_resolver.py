...
class AnnotationResolver:
    ...
    def resolve(self, annotation, solver, module, generics_map=None, annotated=False):
        ...
        if isinstance(annotation, (ast.Name, ast.Str)):
            ...
            key = (module, id)
            if generics_map is not None and (
                annotated or key not in self.z3_types.config.type_vars
            ):
                if id in generics_map:
                    return generics_map[id]
                if id not in self.type_var_poss:
                    raise ValueError(
                        "Invalid type annotation {} in line {}".format(
                ...
                possible_types = [
                    self.resolve(x, solver, module, generics_map)
                    for x in self.type_var_poss[id]
                ]

                if possible_types:
                    solver.add(
                        Or([result_type == x for x in possible_types]),
                        fail_message="Generic type in line {}".format(
                            annotation.lineno
                        ),
                    )
                return result_type
        ...
        if isinstance(annotation, ast.Subscript):
            if not (
                ...
                and isinstance(annotation.slice, ast.Index)
            ):
                raise TypeError(
                    "Invalid type annotation in line {}.".format(annotation.lineno)
                )

            annotation_val = annotation.value.id
            if annotation_val == "List":
                # Parse List type
                return self.z3_types.list(
                    self.resolve(
            ...
            if annotation_val == "Dict":
                # Parse Dict type
                if not (
                    isinstance(annotation.slice.value, ast.Tuple)
                    and len(annotation.slice.value.elts) == 2
                ):
                    raise TypeError(
                        "Dict annotation in line {} should have 2 comma-separated args".format(
                            annotation.lineno
                        )
                    )

                # Get the types of the dict args
                keys_type = self.resolve(
                    annotation.slice.value.elts[0],
                    solver,
                    module,
                    generics_map=generics_map,
                    annotated=annotated,
                )
                vals_type = self.resolve(
                    annotation.slice.value.elts[1],
                    solver,
                    module,
                    generics_map=generics_map,
                    annotated=annotated,
            ...
            if annotation_val == "Tuple":
                ...
                if not isinstance(annotation.slice.value, (ast.Name, ast.Tuple)):
                    raise TypeError(
                        "Invalid tuple type annotation in line {}".format(
                            annotation.lineno
                        )
                    )
                ...
                if isinstance(annotation.slice.value, ast.Name):
                    ...
                    tuple_args_types = [
                        ...
                        for x in annotation.slice.value.elts
                    ]
            ...
            if annotation_val == "Callable":
                ...
                try:
                    ...
                    raise TypeError(
                        "Callable annotation in line {} should be in the format:"
                ...
                args_types = [
                    ...
                    for x in args_annotations
                ]
                return_type = self.resolve(
                    annotation.slice.value.elts[1],
                    solver,
                    module,
                    generics_map=generics_map,
                ...
                return self.z3_types.funcs[len(args_types)](
                    *([0] + args_types + [return_type])
                )
            ...
            if annotation_val == "Union":
                ...
                if not isinstance(annotation.slice.value, (ast.Name, ast.Tuple)):
                    raise TypeError(
                        "Invalid union type annotation in line {}".format(
                            annotation.lineno
                        )
                    )
                ...
                solver.add(
                    Or([result_type == arg for arg in union_args_types]),
                    fail_message="Union in type annotation in line {}".format(
                        annotation.lineno
                    ),
                )
            ...
            if annotation_val in self.z3_types.all_types:
                func = getattr(
                    self.z3_types.type_sort, "class_{}".format(annotation_val)
                )
                if isinstance(annotation.slice.value, ast.Name):
                    args = self.resolve(
                        annotation.slice.value,
    ...
    def get_annotated_function_axioms(
        self, args_types, solver, annotated_function, result_type
    ):
    ...
    def add_type_var(self, target, type_var_node, solver, module):
        ...
        if not type_var_node.args:
            raise TypeError(
                "Invalid type variable declaration in line {}.".format(
                    type_var_node.lineno
                )
            )
        args = type_var_node.args
        if not isinstance(args[0], ast.Str):
            raise TypeError(
                "Name of type variable in line {} should be a string".format(
                    type_var_node.lineno
                )
            )
        ...
        if type_var_node.keywords and type_var_node.keywords[0].arg == "bound":
            ...
            solver.add(
                solver.z3_types.upper(type_var_type) == bound,
            ...
            if len(type_var_possibilities) == 1:
                raise TypeError(
                    "A single constraint is not allowed in TypeVar in line {}".format(
                        type_var_node.lineno
                    )
                )
            possibilities = [
                solver.z3_types.upper(type_var_type) == self.resolve(p, solver, module)
                for p in type_var_possibilities
            ]
            solver.add(
                Or(*possibilities),
                fail_message="Options of type variable {}".format(type_var_name),
            )
        else:
    ...
    def unparse_annotation(
        self, z3_type, context_name=None, lineno=0, definition_linenos=None
    ):
        ...
        if match:
            ...
            return "List[{}]".format(
                self.unparse_annotation(
                    list_type, context_name, lineno, definition_linenos
                )
            )
        ...
        if match:
            ...
            if annotation in definition_linenos and (
                lineno <= definition_linenos[annotation] or context_name == annotation
            ):
        ...
        if match:
            ...
            return "Type[{}]".format(
                self.unparse_annotation(
                    instance, context_name, lineno, definition_linenos
                )
            )
        ...
        if match:
            ...
            for i in range(tuple_len):
                ...
                arg_accessor = getattr(
                    self.z3_types.type_sort, "tuple_{}_arg_{}".format(tuple_len, i + 1)
                )
                arg = simplify(arg_accessor(z3_type))
                args.append(
                    self.unparse_annotation(
                        arg, context_name, lineno, definition_linenos
                    )
                )
        ...
        if match:
            ...
            return "Set[{}]".format(
                self.unparse_annotation(
                    set_type, context_name, lineno, definition_linenos
                )
            )
        ...
        if match:
            ...
            return "Dict[{}, {}]".format(
                self.unparse_annotation(
                    key_type, context_name, lineno, definition_linenos
                ),
                self.unparse_annotation(
                    val_type, context_name, lineno, definition_linenos
                ),
            )
        ...
        if match:
            ...
            for i in range(args_len):
                arg_accessor = getattr(
                    self.z3_types.type_sort, "func_{}_arg_{}".format(args_len, i + 1)
                )
                arg = simplify(arg_accessor(z3_type))
                args.append(
                    self.unparse_annotation(
            ...
            return_accessor = getattr(
                self.z3_types.type_sort, "func_{}_return".format(args_len)
            )
            return_type = simplify(return_accessor(z3_type))
            return_annotation = self.unparse_annotation(
                return_type, context_name, lineno, definition_linenos
            )
